    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
			...
			case R.id.action_load:
                checkDialog("Load from Gallery", "Are you sure you want to clear your drawing and load from Galary?", 3);
                break;
            default:
...
//        getActivity().supportInvalidateOptionsMenu();
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == SELECT_PICTURE) {
			...
			int widht = drawView.getMeasuredWidth();
			int height = drawView.getMeasuredHeight();
			
			drawView.setBackgroundDrawable(new BitmapDrawable(getActivity().getResources(), lessResolution(picturePath, widht, height)));
//                img.setImageURI(selectedImageUri);
	...
	public void checkDialog(String title, String message, final int type) {
		View v = getActivity().getWindow().getDecorView();
        v.setId(1);
        FragmentManager fragmentManager = getActivity().getSupportFragmentManager();
	...
	public void clearDialog() {
        ...
        v.setId(1);
        FragmentManager fragmentManager = getActivity().getSupportFragmentManager();
        BlurredAlertDialog newFragment = BlurredAlertDialog.newInstance("Clear canvas", "You will lose your drawing! Are yoy sure?");
        newFragment.setBlurredAlertDialogListener(new BlurredAlertDialog.BlurredAlertDialogListener() {
            @Override
            public void onBlurredAlertDialogPositiveClick(DialogFragment dialog) {
				drawView.clear();
	...
	public void saveDialog() {
        ...
        v.setId(1);
        FragmentManager fragmentManager = getActivity().getSupportFragmentManager();
        BlurredAlertDialog newFragment = BlurredAlertDialog.newInstance("Save drawing", "Save drawing to device Gallery?");
        newFragment.setBlurredAlertDialogListener(new BlurredAlertDialog.BlurredAlertDialogListener() {
            @Override
            public void onBlurredAlertDialogPositiveClick(DialogFragment dialog) {
                saveToGalery();
